...
{
	cursor_y--;        // Move cursor back a line
...
void putt(RTC*rtc)
{
   ...
   if (rtc->hours < 10)
     putd(0);
                putd(rtc->hours+2);//Hours
putc(':');
    if (rtc->minutes < 10)
      putd(0);
              putd(rtc->minutes);//Minutes
putc(':');
    if (rtc->seconds < 10)
        putd(0);
               putd(rtc->seconds);//Seconds
putc(' ');
    if (rtc->days < 10)
        putd(0);
         putd(rtc->days);//Days
putc('/');
    if (rtc->months < 10)
     putd(0);
        putd(rtc->months);//Months
putc('/');
            putd(rtc->centuries * 100 + rtc->years);//Years

}
void monitor_install(void)
